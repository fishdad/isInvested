...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    //获取本地的密码
   NSString *localpassone = [[NSUserDefaults standardUserDefaults]objectForKey:[Util passwordone]];
    NSString *localpasstwo = [[NSUserDefaults standardUserDefaults]objectForKey:[Util passwordtwo]];
    if (!localpassone||!localpasstwo) {
        if (!localpassone) {
            //保存用户绘制的密码
            [[NSUserDefaults standardUserDefaults]setObject:password forKey:[Util passwordone]];
            ...
            //保存用户绘制的密码
            [[NSUserDefaults standardUserDefaults]setObject:password forKey:[Util passwordtwo]];
            if ([self.delegate respondsToSelector:@selector(setPassWordSuccess:)]) {
...
- (void)confirmPassWord:(NSString *)password{
    ...
    if ([self.delegate respondsToSelector:@selector(unlockView:withPassword:)]) {
        ...
        if ([self.delegate unlockView:self withPassword:password]) {
            ...
            // 密码不正确
            if (self.errorPassWorfBlock) {
                self.errorPassWorfBlock();
            }

...
- (void)initialize {
    self.layer.borderColor = [OrangeBackColor CGColor];
...
- (void)drawProgress:(CGContextRef)context withFrame:(CGRect)frame {
    ...
    UIBezierPath *roundedRect = [UIBezierPath bezierPathWithRoundedRect:insetRect byRoundingCorners:(UIRectCornerTopLeft | UIRectCornerBottomLeft) cornerRadii:CGSizeMake(self.borderRadius.floatValue, self.borderRadius.floatValue)];
    if ([self.flat boolValue] > 0) {
...
- (void)drawStripes:(CGContextRef)context inRect:(CGRect)rect {
    CGContextSaveGState(context);
    [[UIBezierPath bezierPathWithRoundedRect:rect byRoundingCorners:(UIRectCornerTopLeft | UIRectCornerBottomLeft) cornerRadii:CGSizeMake(self.borderRadius.floatValue, self.borderRadius.floatValue)] addClip];
...
- (void)drawRightAlignedLabelInRect:(CGRect)rect {
        ...
        label.text = self.values;
        label.font = [UIFont boldSystemFontOfSize:(15-self.progressInset.floatValue*1.75)];
...
- (UIImage *)gradientProgress {
    if (!_gradientProgress) {
        ...
        NSArray *colors = @[(__bridge id)[UIColor greenColor].CGColor, (__bridge id)[[[self.color darkerColor] darkerColor] colorWithAlphaComponent:0.3].CGColor, (__bridge id)[UIColor clearColor].CGColor];
        CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (__bridge CFArrayRef) colors, locations);
